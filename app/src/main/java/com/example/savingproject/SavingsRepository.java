...
public class SavingsRepository {
    ...
    public void addSaving(SavingsGoal goal, Callback<SavingsGoal> callback) {
        apiService.addSaving(goal).enqueue(callback);
    }

    public void updateSaving(SavingsGoal goal, Callback<SavingsGoal> callback) {
        if (goal.getId() != null) {
            apiService.updateSaving(goal.getId(), goal).enqueue(callback);
        }
    }

    public void deleteSaving(int id, Callback<Void> callback) {
        apiService.deleteSaving(id).enqueue(callback);
    }
}
